...
const SCENE_FINISH_FILE = "res://scena_finish.tscn"
...
func game_victory():
	var scena_finish_instance = preload(SCENE_FINISH_FILE).instantiate()
	scena_finish_instance.set_title("HAS GANADO")
	add_child(scena_finish_instance)


func _on_personaje_player_hit():
	print("Game controller se ha enterado que el jugador sufrio daño")
	if player_health > 0:
